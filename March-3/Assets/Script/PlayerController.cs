    ...
    void Update()
    {
        ...
        foreach (GameObject badguy in badguys)
        {
            ...
            if ((audioData[audioArrayIdx] % 2) == 0 && audioData[audioArrayIdx] != 0)
            {
                ...
                Change = changeAmount * audioData[audioArrayIdx];
            }
            else if ((audioData[audioArrayIdx] % 2) == 1)
            {
               
                Change = -1 * changeAmount * audioData[audioArrayIdx];
            }

            Vector2 calcPos = badguy.transform.right * Time.deltaTime * Change;
            calcPos.y= badguy.transform.position.y;
            badguy.transform.position = calcPos;
